    ...
    firebase.database().ref("stories/" + window.location.href.split("?")[1]).once("value", function(snapshot) {
      if (snapshot.val()) {
          ...
          if(localStorage.name == snapshot.val().author) {
            document.getElementById("story-page").innerHTML += "<button class='btn-primary' onclick='editStory(\"" + window.location.href.split("?")[1] + "\")' style='right: 5px;'><i class='fas fa-edit'></i></button> <a class='btn-primary' id='Download' style='right: 5px;' download='" + snapshot.val().title  + ".txt' href='data:text/plain;charset=utf-8" + snapshot.val().content + "'><i class='fas fa-download'></i></a>";
          }
      } else {
